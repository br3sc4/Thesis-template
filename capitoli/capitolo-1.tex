...
può calcolare i dati di \emph{code coverage} consentendo al \emph{team} di valutare la qualità del codice prodotto. La porzione di codice sottostante è un esempio \emph{test} scritto durante lo sviluppo.
...
\begin{lstlisting}[caption=Esempio di \emph{test}, captionpos=b]
  describe("Testing SupplierService", () => {
    ...
    beforeAll(async () => {
      await container.get(MongoConnection).init();
    ...
    afterAll(() => {
      return container.get(MongoConnection).close();
    })
