...
\emph{Test-Driven Development}. Per l'azienda è fondamentale mantenere il lavoro ben organizzato, per questo motivo è stato definito un \emph{workflow} da seguire per la gestione \
delle \emph{issues}. \\

Quando una \emph{issue} viene presa in carico, per prima cosa, viene asseganto lo stato \emph{"Doing"}; viene specificato lo sviluppatore a cui è assegnata e, se non è già presente, \
viene asseganta la \emph{milestone} dello \emph{sprint} corrente. Prima di iniziarne l'implementazione si devono creare una \emph{merge request} e un \emph{feature branch}, solo dopo \
...
\emph{test}, se fallisce si procede a scrivere il codice necessario per farlo passare, si eseguono tutti i \emph{test} e solo se tutti hanno successo si procede con il \
\emph{refactoring}, se necessario. Una volta completata l'implementazione, lo sviluppatore modifica lo stato della \emph{issue} in \emph{"To Review"} e richiede una \emph{review}. \
...
\emph{way of working} di Wavelop, vengono utilizzate anche per discutere di ciò che è stato implementato e di mofiche da apportare. Quando si prende in carico una \emph{issue}, si \
...
\begin{itemize}
  ...
  \item \emph{Google Docs Suite} per la redazione di documentazione e per la realizzazione di presentazioni.
\end{itemize}
